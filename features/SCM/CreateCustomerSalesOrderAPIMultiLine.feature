Feature: Create Customer Sales Order Using API and Perform Full and Partial shipment
    ...
    ######## IMPORTANT SECOND TAG SHOULD BE - APPLICATION NAME EX: @BSM,@REALITY etc ###################

    @api @BSM
    Scenario: Create Multi Line Customer Sales Order for "product", "product1" for branch "DestinationOrg"
    ...
    @SCM @BSM
    Scenario: Validate Customer Sales Order Status  in SaaS after Shipped
        Given User login into SCM application
        When User Clicks on Home Icon
        And User navigate to Inventory Management

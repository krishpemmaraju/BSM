...
Given('User Connect to DB', async function () {
     ...
     //connectionHJ = await new DBConnection().connectToDB(data.HIGHJUMP_STG[0].DBTYPE, data.HIGHJUMP_STG[0].HJSTGUSERNAME, data.HIGHJUMP_STG[0].HJSTGPASSWORD, data.HIGHJUMP_STG[0].HJSTGALIAS);
});


Then('User should see the successful connect message', async function () {
     // const getHeaders = await new DBConnection().getColumnHeaders(connectionHJ, "SELECT * FROM ods_lookups WHERE LKP_SK = '134452'");
     // const result = await new DBConnection().executeQuery(connectionHJ, "SELECT * FROM ods_lookups WHERE LKP_SK = '134452'");
     ...
     // await new DBConnection().writeMulQueryResultsToFile(connectionHJ, queries, "./src/data/", 'xlsx');
});